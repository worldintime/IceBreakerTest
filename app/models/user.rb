class User < ActiveRecord::Base
  ...
  def search_results(current_user_id)
    status = Conversation.where("status = ? AND sender_id = ? AND receiver_id = ? OR status = ? AND sender_id = ? AND receiver_id = ?",
                                'Closed', current_user_id, self.id, 'Closed', self.id, current_user_id ).to_a
